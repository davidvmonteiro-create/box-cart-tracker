import { Package, ShoppingCart, MapPin, AlertTriangle } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { mockBoxes, mockCarts, mockCorridors } from '@/data/mockData';

const stats = [
  { label: 'Total Caixas', value: mockBoxes.length, icon: Package, color: 'bg-primary' },
  { label: 'Carrinhos Ativos', value: mockCarts.filter(c => c.boxes.length > 0).length, icon: ShoppingCart, color: 'bg-accent' },
  { label: 'Corredores', value: mockCorridors.length, icon: MapPin, color: 'bg-info' },
  { label: 'Caixas Soltas', value: mockBoxes.filter(b => !b.cartId).length, icon: AlertTriangle, color: 'bg-warning' },
];
                ...
                <div key={cart.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{cart.boxes.length} caixas</p>
                    </div>
                  </div>
